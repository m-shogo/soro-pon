...
| B13-A11Y-AUTO | React/Vitest/Playwright | semantic selection/dialog/focus/rotation | unit 432 and visual 80 green | PASS | automated scope |
| B13-INTEGRITY | exact product SHA candidate | persistence/import contracts green | 101/101 | PASS | local Node 24 |
| B13-BUILD | exact product SHA candidate | typecheck/build/deterministic inventory | aggregate `75be0293…`; 48 files; no maps | PASS | local macOS; ignored `.DS_Store` excluded |
| B13-PYTHON | local + exact-SHA CI | asset fixtures green | 92/92 local 3.14.5; CI 3.13 green with `pip check` | PASS | local version differs; exact 3.13 claim is CI |
| B13-CI | GitHub PR exact SHA | CI green | run `30237605574` SUCCESS | PASS | exact SHA `2a447930…` |
| B13-CI-INTEGRITY | GitHub PR exact SHA | workflow green | run `30237605563` SUCCESS | PASS | exact SHA `2a447930…` |
...
| B13-SAFARI-ROTATION | same | ≥20 cycles and ≥20 min | 4 cycles / 190 sec | BLOCKED | restore Safari AX windows; rerun full threshold |
| B13-SAFARI-DIAGNOSTICS | same | console/page/network captured | unavailable | NOT TESTED | enable WebDriver/Develop inspection |
| B13-VOICEOVER | real VoiceOver + Safari | mandatory major flow | Safari web content + TOP captions only | BLOCKED | restore Safari AX windows; traverse full checklist |
...
| B13-PRODUCT-DEFECT | executed scope | open = 0 | fixed 3; open 0 | PASS | unexecuted real-environment scope excluded |
